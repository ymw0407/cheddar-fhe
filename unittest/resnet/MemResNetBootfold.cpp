// Bootfold driver — 부트스트랩 지연 배치(lazy placement) 스파이크 (2026-08-12).
// 규약: 회로를 바꾸는 실험은 공유 드라이버를 건드리지 않고 별도 파일.
//
// 아이디어(HyPHEN §3.5 의 배치 사고방식 차용): 블록은 레벨을 in->in-4 로 4개만
// 소모하는데 기존 드라이버(MemResNetBank)는 매 블록 전에 end_level(8) 재충전
// = 매 블록 부트 1회. 레벨 예산이 남아 있으면 부트 없이 다음 블록을 이어 붙여
// (8->4->0) 블록 2개당 부트 1회로 줄인다. 가중치 플레인텍스트는 생성 시점에
// 레벨이 고정되므로, 케이던스에 맞춰 블록을 in_level=8 또는 4 변형으로 만든다.
//
// env:
//   BOOTFOLD=1(기본)  lazy 배치 / BOOTFOLD=0  매 블록 부트 (bank 드라이버와
//                     동일 산술 — 같은 바이너리로 A/B 회귀 확인용)
//   MIN_IN=4          블록이 요구하는 최소 입력 레벨 (deg3 소모 4 기준)
// 리스크(스모크가 판정): 레벨 0 에서의 부트 진입, 저레벨 구간 정밀도.
// 판정선: 500장 lazy vs per-block Δacc >= -0.5pp AND 순추론 단축 확인 → 일지.
//
// Circuit / scale / manifest 규약은 MemResNet.cpp 주석 참조 (동일).
...
TEST_P(Testbed32, MemResNetBootfold) {
  ...
  const bool lazy = !getenv("BOOTFOLD") || atoi(getenv("BOOTFOLD")) != 0;
  const int min_in = getenv("MIN_IN") ? atoi(getenv("MIN_IN")) : 4;
  std::cout << "MEMRESNET_DIR=" << dir << "  scale_S=" << S
            << "  [bootfold driver, " << (lazy ? "lazy" : "per-block")
            << " placement, MIN_IN=" << min_in << "]" << std::endl;
  ...
  // ---- compressed blocks (MemBlockBank) + kept BasicBlocks (hybrid) ------
  // bootfold: 다음 블록의 in_level 을 "지금 암호문이 서 있을 레벨"로 고정 생성.
  // 레벨이 min_in 아래로 내려가면 그 블록 앞이 부트 지점(in_level=end_level).
  struct AnyBlock {
    std::unique_ptr<MemBlockBank<word>> mem;
    std::unique_ptr<ConvBN<word>> c1, c2, sc;  // kept BasicBlock parts
    TensorLayout out;
  };
  std::vector<AnyBlock> blocks;
  TensorLayout cur{32, 1, 16};
  int lv = -1;  // 첫 블록은 무조건 부트 지점 (relu 출력 레벨이 얕음)
  for (const auto &m : metas) {
    ...
    if (m.type == "basic") {
      ...
      ab.out = mid;
      lv = relu->OutputLevel();  // kept 블록의 출구는 relu 가 결정
      ...
      const int in_lv = (lazy && lv >= min_in) ? lv : end_level;
      std::cout << "[build] " << m.name << " in{" << cur.width << "," << cur.pack
                << "," << cur.channels << "} rP_coef(" << coef.size() << "):";
      for (double c : coef) std::cout << " " << c;
      std::cout << std::flush;
      ab.mem = std::make_unique<MemBlockBank<word>>(
          boot_context, cur, m.out_ch, m.stride, m.rdown_k, m.b,
          load(p + "__rdown_w"), load(p + "__rdown_b"),
          coef, load(p + "__rup_w"), sc, S, in_lv,
          m.n_bank, qkw, qkb, pcoef, vw);
      ASSERT_GE(ab.mem->OutLevel(), 0)
          << m.name << " underflows levels (in " << in_lv << ") — raise MIN_IN";
      ab.out = ab.mem->OutLayout();
      lv = ab.mem->OutLevel();
      std::cout << " -> level " << in_lv << "->" << ab.mem->OutLevel()
                << (in_lv == end_level ? "  [boot-in]" : "  [chained]")
                << (ab.mem->HasMemory()
                        ? "  [bank on, " + std::to_string(ab.mem->BankParts()) +
                              " parts]"
                        : "")
                << std::endl;
    }
  ...
  // 데이터 유무는 키 생성 前에 확인 (실패를 3분 뒤에 알지 않도록)
  ...
  // KEYS_RESIDENT=1: 그룹 키 전부를 셋업에서 한 번에 생성 (bank 드라이버와 동일)
  ...
  Ct main_ct;
  // 위상별 시간 분해 + 블록 구간 부트 집계 (relu 내부 부트는 양 모드 동일이라 제외)
  uint32_t img_boots = 0;
  auto adj = [&](Ct &ct, int target) {
    if (boot_context->param_.NPToLevel(ct.GetNP()) < target) ++img_boots;
    AdjustLevel(boot_context, ct, target, interface_->GetEvkMap());
  };
  ...
  for (int i = 0; i < num_test_images; i++) {
    ...
    __ProfileStart("MemResNetBootfold", warm_up,
    ...
    for (size_t bi = 0; bi < blocks.size(); bi++) {
      ...
      if (blocks[bi].mem) {
        adj(main_ct, blocks[bi].mem->InLevel());
    ...
    adj(main_ct, kFcLevel);
    fc.Evaluate(main_ct, main_ct, interface_->GetEvkMap());
    __ProfileEnd("MemResNetBootfold");
    ...
    std::cout << "[time-breakdown] conv1+relu " << phase_ms(tb0, tb1)
              ...
              << std::endl;
    std::cout << "[bootfold] block/tail boots this img = " << img_boots
              << "  (" << (lazy ? "lazy" : "per-block") << ")" << std::endl;
